...
def sma(df=None,ax=None,sw=5,lw=25):
  s = f'sma{sw}'
  l = f'sma{lw}'
  ...
  return df
##指数平滑移動平均
def ema(df=None,ax=None,sw=5,lw=25):
  sema = f'ema{sw}'
  lema = f'ema{lw}'
  ...
  # バックテストの結果を出力
##移動平均収束拡散法
def macd(df=None,sw=12,lw=26,sig=9,ax=None):
  sw = f'ema{sw}'
  lw = f'ema{lw}'
...
#ボラティリティ系
##ボリンジャーバンド
def bband(df=None,ax=None,n=25):
  # N期間の移動平均線(SMA)を計算
